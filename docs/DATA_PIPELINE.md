# Clinical Knowledge Ingestion & Normalization Pipeline

## 1. Overview
The Healthcare Clinical Decision Support (CDS-RAG) ingestion pipeline transforms raw, heterogeneous medical sources into normalized, deduplicated, and provenance-tracked knowledge documents and semantic chunks.
...
### Stage 1: Adapter Extraction & Normalization
...
### Stage 4: Incremental Vector Synchronization
...
Every ingestion run produces an auditable `manifest.json` in `rag_module/data/artifacts/<source_id>/` and `rag_module/data/artifacts/combined/` documenting:
- Exact document and chunk counts
- Execution duration and timestamps
- Embedding model version and dimension
- Content hash summaries
